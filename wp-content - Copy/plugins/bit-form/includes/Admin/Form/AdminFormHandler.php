...
{$sm_styles}
  ...
  public function updateForm($Request, $post)
  {
    ...
    if (!empty($formSettings->formAbandonment)) {
      ...
      if (isset($abandonmentInteg->id)) {
        ...
        $abandonmentIntegDetails = json_encode($formAbandonment);
  ...
  public function savePaymentSetting($Request, $post)
  {
    ...
    $canSave = null;
    // $checkedCanSave = [
    //   'PayPal'   => $this->addWebhookToPaypal($paySetting),
    //   'Razorpay' => true,
    //   'Stripe'   => $this->addWebhookToStripe($paySetting),
    // ];
    // $canSave = $checkedCanSave[$paySetting->type];
    ...
    if (is_null($canSave)) {
      return ['message' => __('Please re-check your Client ID & Secret', 'bit-form')];
    }
  ...
  {
    // $transaction_mode = $paySetting->mode;
    // $publishableKey = $paySetting->publishableKey;
    $clientSecret = $paySetting->clientSecret;
