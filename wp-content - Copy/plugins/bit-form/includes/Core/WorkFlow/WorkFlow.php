...
final class WorkFlow
{
  ...
  {
    $workFlows = $this->getWorkFlow(['create_edit', $workFlowRun], ['onload'], null, 'workflow_order');
  ...
  {
    $workFlows = $this->getWorkFlow(['create_edit', $workFlowRun], ['oninput'], null, 'workflow_order');
    $workFlowReturnable = [];
    $data = SmartTags::getPostUserData();
    $workFlowReturnable['smart_tags'] = SmartTags::smartTags($data);
    if (empty($workFlows) || is_wp_error($workFlows)) {
      return $workFlowReturnable;
    }
    $onUserInputRun = [];
