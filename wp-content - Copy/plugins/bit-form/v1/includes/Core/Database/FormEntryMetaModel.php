...
use BitCode\BitForm\Core\Database\Helper;
...
class FormEntryMetaModel extends Model {
    ...
    public function getEntryMeta($formFields, $entries, $limit = null, $offset = null, $filter = null, $sortBy = null, $fieldConditions = null) {
        $entry_table = $this->app_db->prefix . 'bitforms_form_entries';
    ...
    public function getExportEntry($formFields, $entries, $formId, $fieldLabels, $fileFormate, $limit = null, $sortBy = null, $sortByField = null) {
        $entry_table = $this->app_db->prefix . 'bitforms_form_entries';
        ...
        foreach ($result as $key => $value) {
            foreach ($formFieldsNames as $formFieldName) {
                ...
                if ($formFieldName === '__user_id' && intval($value->$formFieldName) > 0) {
                    $allData[$key][$formFieldName] = $userNames[$value->$formFieldName];
                } elseif ($formFieldName === '__user_ip') {
                    $allData[$key][$formFieldName] = long2ip($value->$formFieldName);
                } else {
                    $allData[$key][$formFieldName] = preg_replace('/[\]["]/i', '', $value->$formFieldName);
                }
                if (is_string($value->$formFieldName) && in_array($fileFormate, ['csv', 'xlsx', 'xls'])) {
                    $allData[$key][$formFieldName] = $this->csvInjectionPrevent($value->$formFieldName);
                }
