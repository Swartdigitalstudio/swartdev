...
GLOBALOBJ;
    ...
    if (isset($this->_fields['button'])) {
      foreach ($this->_fields['button'] as $button) {
        if ('save-draft' === $button['field']->btnTyp) {
          $neededFiles = ScriptFilePriorityManager::formAbandonmentNeededFiles('draftButton');
          $allFiles = array_merge($allFiles, $neededFiles);
          break;
        }
      }
  ...
  {
    $customFlds = ['select', 'country', 'currency', 'phone-number', 'file-up', 'advanced-file-up', 'paypal', 'razorpay', 'stripe', 'recaptcha', 'repeater', 'signature'];
    ...
    $scriptLoaderFields = ['paypal', 'razorpay', 'stripe', 'recaptcha'];
